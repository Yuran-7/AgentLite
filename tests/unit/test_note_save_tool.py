...
from agent_lite.core.tools.builtin.note_save import NoteSaveTool
...
async def test_note_save_appends_note(tmp_path: Path) -> None:
    ...
    tool = NoteSaveTool(store, SESSION_ID, "run-1")

    result = await tool.invoke({"content": "Python 3.12"})

    assert result.content == "saved"
    assert not result.is_error
    assert "Python 3.12" in store.read_notes(SESSION_ID)
